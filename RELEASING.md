...
- `release_tag`: the target tag, for example `v1.11.3`.
...
```powershell
Get-FileHash .\AudioForge-v1.11.3-win64-ultra.7z -Algorithm SHA256
```
...
.\.venv\Scripts\python.exe python\tools\release_provenance.py create `
  ...
  --archive .\AudioForge-v1.11.3-win64-ultra.7z `
...
.\.venv\Scripts\python.exe python\tools\release_provenance.py verify `
  --bundle .\dist\AudioForge `
  --archive .\AudioForge-v1.11.3-win64-ultra.7z `
  --checksum .\AudioForge-v1.11.3-win64-ultra.7z.sha256 `
  --manifest .\AudioForge-v1.11.3-win64-ultra.7z.manifest.json `
  --metadata .\AudioForge-v1.11.3-win64-ultra.7z.metadata.json `
  --baseline .\evaluation\release-bundle-path-baseline.json
```
...
2. Create annotated tag `v1.11.3`.
...
4. Push `master` and `v1.11.3`, or run the `Release package` workflow manually
