...
maps to MSI `1.12.1`; the final `1.12.1` maps to MSI `1.12.199`. RC sequences are bounded
to 1 through 98 and patch components to 654. MSI's numeric value is an internal
installer identity; app and release metadata retain the public `1.12.1` version.
...
- `release_tag`: the target tag, for example `v1.12.1`.
...
binds its measurements to the archive SHA-256. The v1.12.1 hardware gate
...
unqualified cases; they are not v1.12.1 release verification claims or
...
```powershell
Get-FileHash .\AudioForge-v1.12.1-win64-ultra.7z -Algorithm SHA256
```
...
.\.venv\Scripts\python.exe python\tools\release_provenance.py create `
  ...
  --archive .\AudioForge-v1.12.1-win64-ultra.7z `
...
.\.venv\Scripts\python.exe python\tools\release_provenance.py verify `
  --bundle .\dist\AudioForge `
  --archive .\AudioForge-v1.12.1-win64-ultra.7z `
  --checksum .\AudioForge-v1.12.1-win64-ultra.7z.sha256 `
  --manifest .\AudioForge-v1.12.1-win64-ultra.7z.manifest.json `
  --metadata .\AudioForge-v1.12.1-win64-ultra.7z.metadata.json `
  --baseline .\evaluation\release-bundle-path-baseline.json
```
...
2. Create annotated tag `v1.12.1`.
...
4. Push `master` and `v1.12.1`, or run the `Release package` workflow manually
...
6. On a temporary or standing self-hosted runner labelled `self-hosted`,
   ...
   compatibility evidence but are not mandatory v1.12.1 gates.
