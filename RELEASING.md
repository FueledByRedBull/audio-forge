...
- `release_tag`: the target tag, for example `v1.8.9`.
...
```powershell
Get-FileHash .\AudioForge-v1.8.9-win64-ultra.7z -Algorithm SHA256
```

Manual publish:

1. Commit tracked source/doc/version changes.
2. Create annotated tag `v1.8.9`.
...
4. Push `master` and `v1.8.9`, or run the `Release package` workflow manually with `upload_to_release` enabled.
